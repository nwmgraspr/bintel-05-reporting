...
def create_restaurant_reporting_view(
    ...
    WHY: The reporting view combines order facts with
    customer and menu dimensions. This creates one
    business-ready dataset for analysis.
...
def slice_restaurant_sales_by_region(
    ...
    """Slice restaurant sales by one region.

    WHY: A slice selects one value from one dimension.
    Here, we select one customer region and compare
    restaurant sales by menu category.

    Args:
        conn: Open DuckDB connection.
        selected_region: Region to analyze.

    Returns:
        DataFrame with Category and TotalSales.
    """

    LOG.info(f"OLAP slice: restaurant sales for Region = {selected_region!r}")
    ...
    if df_slice.empty:
        raise ValueError(f"No restaurant sales found for region {selected_region!r}")
...
def dice_restaurant_sales_by_dimensions(
    ...
    """Dice restaurant sales by selected regions and menu categories.

    WHY: A dice filters across multiple dimensions.
    Here, we compare selected customer regions and
    menu categories.
    ...
    Returns:
        DataFrame with Region, Category, and TotalSales.
    """

    LOG.info(f"OLAP dice: Regions={selected_regions}; Categories={selected_categories}")
...
def rollup_restaurant_sales_by_time(
    ...
    WHY: Roll-up moves from detailed order data to
    broader summaries. This helps managers see
    monthly, quarterly, yearly, and total performance.
    ...
    Returns:
        DataFrame containing time-based summaries.
    """
...
def summarize_restaurant_results(
    ...
    # Strongest region-category combination.
    best_region: str = str(df_dice.iloc[0]["Region"])
...
# === Section 2.3 DEFINE A CREATE REPORTING VIEW FUNCTION ===
